...
app.post('/api/wallet/create', async (req, res) => {
  try {
    ...
    console.log(`Wallet created and funded for: ${publicKey}`);
...
app.post('/api/transaction/send-payment', async (req, res) => {
  ...
  try {
    // 1. Retrieve the source account's secret key from your secure storage.
    const senderWallet = userWallets[sourcePublicKey];
    // if (!senderWallet) {
    //   return res.status(404).json({ error: 'Source wallet not found.' });
    // }
    
    const sourceKeypair = StellarSdk.Keypair.fromSecret(senderWallet.secretKey);
    
    // 2. Load the source account from the network to get the sequence number.
    ...
    const transaction = new StellarSdk.TransactionBuilder(account, {
      ...
      .addOperation(StellarSdk.Operation.payment({
        ...
        asset: StellarSdk.Asset.native(), // Send native XLM
        amount: amount.toString(),
      }))
      .setTimeout(30)
      .build();

    // 4. Sign the transaction with the source account's secret key.
